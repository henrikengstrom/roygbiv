...
import roygbiv.worker.{ Stop, RayTracer }
...
class RenderResource extends Actor {
  ...
  def loadScene = {
    ...
    loader ! LoadScene(JsonSceneLoader.SceneType, "/Users/henrikengstrom/code/scala/roygbiv/server/src/main/resources/SceneExample.lcj")
  }
  ...
  def distributeWork(scene: Scene) = {
    ...
    for (i ← 1 to Runtime.getRuntime.availableProcessors) {
      ...
      val actor = actorOf(new RayTracer("127.0.0.1", 2552)).start()
